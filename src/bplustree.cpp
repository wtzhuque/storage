...
#define key(node) ((btree_key *)offset_ptr(node))
#define data(node) ((long *)(offset_ptr(node) + _max_entries * sizeof(btree_key)))
#define sub(node) ((btree_off *)(offset_ptr(node) + (_max_order - 1) * sizeof(btree_key)))
...
static int key_binary_search(struct bplus_node *node, btree_key target)
{
        btree_key *arr = key(node);
...
static inline int parent_key_index(struct bplus_node *parent, btree_key key)
{
...
static struct bplus_node *node_fetch(struct bplus_tree *tree, btree_off offset)
{
...
static struct bplus_node *node_seek(struct bplus_tree *tree, btree_off offset)
{
        if (offset == INVALID_OFFSET) {
                return NULL;
        }
...
static btree_off new_node_append(struct bplus_tree *tree, struct bplus_node *node)
{
...
static inline void sub_node_flush(struct bplus_tree *tree, struct bplus_node *parent, btree_off sub_offset)
{
...
static long bplus_tree_search(struct bplus_tree *tree, btree_key key)
{
        int ret = -1;
...
static int non_leaf_insert(struct bplus_tree *tree, struct bplus_node *node,
                             struct bplus_node *l_ch, struct bplus_node *r_ch, btree_key key);

static int parent_node_build(struct bplus_tree *tree, struct bplus_node *l_ch,
                             struct bplus_node *r_ch, btree_key key)
{
...
static btree_key non_leaf_split_left(struct bplus_tree *tree, struct bplus_node *node,
                	         struct bplus_node *left, struct bplus_node *l_ch,
                	         struct bplus_node *r_ch, btree_key key, int insert)
{
        int i;
        btree_key split_key;
        ...
        memmove(&key(left)[0], &key(node)[0], pivot * sizeof(btree_key));
        memmove(&sub(left)[0], &sub(node)[0], pivot * sizeof(btree_off));

        /* replicate from key[insert] to key[split - 1] in original node */
        memmove(&key(left)[pivot + 1], &key(node)[pivot], (split - pivot - 1) * sizeof(btree_key));
        memmove(&sub(left)[pivot + 1], &sub(node)[pivot], (split - pivot - 1) * sizeof(btree_off));
        ...
        memmove(&key(node)[0], &key(node)[split - 1], (node->children - 1) * sizeof(btree_key));
        memmove(&sub(node)[1], &sub(node)[split], (node->children - 1) * sizeof(btree_off));

        return split_key;
}

static btree_key non_leaf_split_right1(struct bplus_tree *tree, struct bplus_node *node,
                        	   ...
                        	   struct bplus_node *r_ch, btree_key key, int insert)
{
        ...
        btree_key split_key = key(node)[split - 1];
        ...
        memmove(&key(right)[pivot + 1], &key(node)[split], (right->children - 2) * sizeof(btree_key));
        memmove(&sub(right)[pivot + 2], &sub(node)[split + 1], (right->children - 2) * sizeof(btree_off));
...
static btree_key non_leaf_split_right2(struct bplus_tree *tree, struct bplus_node *node,
                        	   struct bplus_node *right, struct bplus_node *l_ch,
                        	   struct bplus_node *r_ch, btree_key key, int insert)
{
        int i;
        ...
        btree_key split_key = key(node)[split];
        ...
        memmove(&key(right)[0], &key(node)[split + 1], pivot * sizeof(btree_key));
        memmove(&sub(right)[0], &sub(node)[split + 1], pivot * sizeof(btree_off));
        ...
        memmove(&key(right)[pivot + 1], &key(node)[insert], (_max_order - insert - 1) * sizeof(btree_key));
        memmove(&sub(right)[pivot + 2], &sub(node)[insert + 1], (_max_order - insert - 1) * sizeof(btree_off));
...
static void non_leaf_simple_insert(struct bplus_tree *tree, struct bplus_node *node,
                        	   ...
                        	   btree_key key, int insert)
{
        memmove(&key(node)[insert + 1], &key(node)[insert], (node->children - 1 - insert) * sizeof(btree_key));
        memmove(&sub(node)[insert + 2], &sub(node)[insert + 1], (node->children - 1 - insert) * sizeof(btree_off));
        /* insert new key and sub-nodes */
        key(node)[insert] = key;
...
static btree_key leaf_split_left(struct bplus_tree *tree, struct bplus_node *leaf,
                	     struct bplus_node *left, btree_key key, long data, int insert)
{
        /* split = [m/2] */
        ...
        memmove(&key(left)[0], &key(leaf)[0], pivot * sizeof(btree_key));
        ...
        memmove(&key(left)[pivot + 1], &key(leaf)[pivot], (split - pivot - 1) * sizeof(btree_key));
        memmove(&data(left)[pivot + 1], &data(leaf)[pivot], (split - pivot - 1) * sizeof(long));

        /* original leaf left shift */
        memmove(&key(leaf)[0], &key(leaf)[split - 1], leaf->children * sizeof(btree_key));
        memmove(&data(leaf)[0], &data(leaf)[split - 1], leaf->children * sizeof(long));

        return key(leaf)[0];
}

static btree_key leaf_split_right(struct bplus_tree *tree, struct bplus_node *leaf,
                	      struct bplus_node *right, btree_key key, long data, int insert)
{
        /* split = [m/2] */
        int split = (leaf->children + 1) / 2;
        ...
        memmove(&key(right)[0], &key(leaf)[split], pivot * sizeof(btree_key));
        ...
        memmove(&key(right)[pivot + 1], &key(leaf)[insert], (_max_entries - insert) * sizeof(btree_key));
...
static void leaf_simple_insert(struct bplus_tree *tree, struct bplus_node *leaf,
                	       btree_key key, long data, int insert)
{
        memmove(&key(leaf)[insert + 1], &key(leaf)[insert], (leaf->children - insert) * sizeof(btree_key));
...
static int leaf_insert(struct bplus_tree *tree, struct bplus_node *leaf, btree_key key, long data)
{
        /* Search key location */
...
static int bplus_tree_insert(struct bplus_tree *tree, btree_key key, long data)
{
        struct bplus_node *node = node_seek(tree, tree->root);
        while (node != NULL) {
                if (is_leaf(node)) {
                        	     ...
                        	     int parent_key_index, int remove)
{
        ...
        memmove(&key(node)[1], &key(node)[0], remove * sizeof(btree_key));
        memmove(&sub(node)[1], &sub(node)[0], (remove + 1) * sizeof(btree_off));
                        	     ...
                        	     int parent_key_index, int remove)
{
        ...
        memmove(&key(left)[left->children], &key(node)[0], remove * sizeof(btree_key));
        memmove(&sub(left)[left->children], &sub(node)[0], (remove + 1) * sizeof(btree_off));

        /* sub-node sum = node->children - 1 */
        memmove(&key(left)[left->children + remove], &key(node)[remove + 1], (node->children - remove - 2) * sizeof(btree_key));
        memmove(&sub(left)[left->children + remove + 1], &sub(node)[remove + 2], (node->children - remove - 2) * sizeof(btree_off));
                        	      ...
                        	      int parent_key_index)
{
        ...
        memmove(&key(right)[0], &key(right)[1], (right->children - 2) * sizeof(btree_key));
        memmove(&sub(right)[0], &sub(right)[1], (right->children - 1) * sizeof(btree_off));
                        	      ...
                        	      int parent_key_index)
{
        ...
        memmove(&key(node)[node->children - 1], &key(right)[0], (right->children - 1) * sizeof(btree_key));
        memmove(&sub(node)[node->children - 1], &sub(right)[0], right->children * sizeof(btree_off));
...
static inline void non_leaf_simple_remove(struct bplus_tree *tree, struct bplus_node *node, int remove)
{
        ...
        memmove(&key(node)[remove], &key(node)[remove + 1], (node->children - remove - 2) * sizeof(btree_key));
        memmove(&sub(node)[remove + 1], &sub(node)[remove + 2], (node->children - remove - 2) * sizeof(btree_off));
        node->children--;
}
                		 ...
                		 int parent_key_index, int remove)
{
        ...
        memmove(&key(leaf)[1], &key(leaf)[0], remove * sizeof(btree_key));
        memmove(&data(leaf)[1], &data(leaf)[0], remove * sizeof(btree_off));
                		 ...
                		 struct bplus_node *left, int parent_key_index, int remove)
{
        ...
        memmove(&key(left)[left->children], &key(leaf)[0], remove * sizeof(btree_key));
        memmove(&data(left)[left->children], &data(leaf)[0], remove * sizeof(btree_off));
        memmove(&key(left)[left->children + remove], &key(leaf)[remove + 1], (leaf->children - remove - 1) * sizeof(btree_key));
        memmove(&data(left)[left->children + remove], &data(leaf)[remove + 1], (leaf->children - remove - 1) * sizeof(btree_off));
                                  ...
                                  int parent_key_index)
{
        ...
        memmove(&key(right)[0], &key(right)[1], (right->children - 1) * sizeof(btree_key));
        memmove(&data(right)[0], &data(right)[1], (right->children - 1) * sizeof(btree_off));
        right->children--;
...
{
        memmove(&key(leaf)[leaf->children], &key(right)[0], right->children * sizeof(btree_key));
        memmove(&data(leaf)[leaf->children], &data(right)[0], right->children * sizeof(btree_off));
...
{
        memmove(&key(leaf)[remove], &key(leaf)[remove + 1], (leaf->children - remove - 1) * sizeof(btree_key));
        memmove(&data(leaf)[remove], &data(leaf)[remove + 1], (leaf->children - remove - 1) * sizeof(btree_off));
        leaf->children--;
}

static int leaf_remove(struct bplus_tree *tree, struct bplus_node *leaf, btree_key key)
{
...
static int bplus_tree_delete(struct bplus_tree *tree, btree_key key)
{
        struct bplus_node *node = node_seek(tree, tree->root);
        while (node != NULL) {
                if (is_leaf(node)) {
...
{
        long start = -1;
        btree_key min = key1 <= key2 ? key1 : key2;
        btree_key max = min == key1 ? key2 : key1;
...
static btree_off str_to_hex(char *c, int len)
{
        btree_off offset = 0;
        while (len-- > 0) {
                if (isdigit(*c)) {
...
static inline void hex_to_str(btree_off offset, char *buf, int len)
{
...
static inline btree_off offset_load(int fd)
{
...
static inline ssize_t offset_store(int fd, btree_off offset)
{
        char buf[ADDR_STR_WIDTH];
...
bplus_tree *bplus_tree_init(const char *filename, int block_size)
{
        int i;
        ...
        _max_order = (block_size - sizeof(node)) / (sizeof(btree_key) + sizeof(btree_off));
        _max_entries = (block_size - sizeof(node)) / (sizeof(btree_key) + sizeof(long));
        if (_max_order <= 2) {
                fprintf(stderr, "block size is too small for one node!\n");
                return NULL;
        }
        ...
        _max_order = (_block_size - sizeof(node)) / (sizeof(btree_key) + sizeof(btree_off));
        _max_entries = (_block_size - sizeof(node)) / (sizeof(btree_key) + sizeof(long));
